...
label continue_chat_after_first:
    if first_choice == 1:
        ...
        $ renpy.pause(0.3)
        $ show_chat_choices(["Звучит здорово! Я согласна на все!", "Давай сначала посмотрим, как пройдет день. Я немного устала сегодня."], second_choice_callback)
        $ store.wait_for_chat()
    elif first_choice == 2:
        ...
        $ renpy.pause(0.3)
        $ show_chat_choices(["Звучит здорово! Библиотека – отличная идея!", "Спасибо, Лина! Я, наверное, пока буду просто наблюдать."], second_choice_callback)
        $ store.wait_for_chat()
    elif first_choice == 3:
