...
init python:
    # Пытаемся инициализировать GUI с правильным разрешением
    # Не используем renpy.notify, так как система уведомлений ещё не готова
    try:
        gui.init(1920, 1080)
    except Exception:
        # Если не получается, пробуем с меньшим разрешением
        try:
            gui.init(1280, 720)
        except Exception:
            # Если и это не работает, используем значения по умолчанию
            # (Ren'Py сам установит разумные значения)
            pass
...
define gui.text_font = "LeticeaBumsteadCyrillic.otf"

## Шрифт, используемый именами персонажей.
define gui.name_text_font = "LeticeaBumsteadCyrillic.otf"

## Шрифт, используемый текстом вне игры.
define gui.interface_text_font = "FOT-YurukaStd-UB.otf"
...
define gui.main_menu_background = "gui/board_menu.png"
